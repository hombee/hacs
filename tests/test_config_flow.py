...
async def test_user_flow_creates_entry(hass: HomeAssistant) -> None:
    with (
        patch(
            "custom_components.hombee.config_flow._probe",
            return_value=True,
        ),
        patch(
            "custom_components.hombee.async_setup_entry",
            return_value=True,
        ),
    ):
...
async def test_duplicate_installation_aborts(hass: HomeAssistant) -> None:
    ...
    with patch("custom_components.hombee.config_flow._probe", return_value=True):
        result = await hass.config_entries.flow.async_init(
            DOMAIN, context={"source": SOURCE_USER}
        )
        result = await hass.config_entries.flow.async_configure(
